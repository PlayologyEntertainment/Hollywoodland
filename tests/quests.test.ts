...
const ALL: QuestDef[] = [SIMPLE_QUEST, GATED_QUEST];
...
describe('evaluateQuestCondition', () => {
  it('delegates fact/resource conditions to the shared evaluator', () => {
    ...
    expect(evaluateQuestCondition(state, { kind: 'resource-at-least', resource: 'money', minimum: 50 }, ALL)).toBe(true);
  });

  it('evaluates a quest-status condition against another quest', () => {
    const state = createDefaultCareerState();
    expect(evaluateQuestCondition(state, { kind: 'quest-status', questId: 'simple', status: 'available' }, ALL)).toBe(true);
    expect(evaluateQuestCondition(state, { kind: 'quest-status', questId: 'simple', status: 'completed' }, ALL)).toBe(false);
  });

  it('treats a dangling quest-status target as non-matching rather than throwing', () => {
    const state = createDefaultCareerState();
    expect(evaluateQuestCondition(state, { kind: 'quest-status', questId: 'missing', status: 'locked' }, ALL)).toBe(false);
  });
});

describe('getQuestStatus', () => {
  it('is locked when prerequisites are unmet', () => {
    expect(getQuestStatus(createDefaultCareerState(), GATED_QUEST, ALL)).toBe('locked');
  });

  it('is available with no prerequisites and not yet started', () => {
    expect(getQuestStatus(createDefaultCareerState(), SIMPLE_QUEST, ALL)).toBe('available');
  });

  it('is active once started but before the final stage completes', () => {
    const started = startQuest(createDefaultCareerState(), SIMPLE_QUEST, ALL);
    expect(getQuestStatus(started, SIMPLE_QUEST, ALL)).toBe('active');
  });

  it('is completed once every stage is complete', () => {
    let state = startQuest(createDefaultCareerState(), SIMPLE_QUEST, ALL);
    state = completeQuestStage(state, SIMPLE_QUEST, 'first');
    state = completeQuestStage(state, SIMPLE_QUEST, 'second');
    expect(getQuestStatus(state, SIMPLE_QUEST, ALL)).toBe('completed');
  });
  ...
  it('unlocks a gated quest once its prerequisite quest is completed', () => {
    ...
    state = completeQuestStage(state, SIMPLE_QUEST, 'first');
    state = completeQuestStage(state, SIMPLE_QUEST, 'second');
    expect(getQuestStatus(state, GATED_QUEST, ALL)).toBe('available');
  });
});
...
describe('getActiveStageIndex / getActiveStage', () => {
  ...
  it('points at the first incomplete stage once started', () => {
    const state = startQuest(createDefaultCareerState(), SIMPLE_QUEST, ALL);
...
describe('startQuest', () => {
  it('no-ops when the quest is locked', () => {
    const state = createDefaultCareerState();
    expect(startQuest(state, GATED_QUEST, ALL)).toBe(state);
  });

  it('is idempotent once already started', () => {
    const started = startQuest(createDefaultCareerState(), SIMPLE_QUEST, ALL);
    expect(startQuest(started, SIMPLE_QUEST, ALL)).toBe(started);
  });
});
...
describe('applyQuestActionById', () => {
  it('no-ops on an unknown quest id', () => {
    const state = createDefaultCareerState();
    expect(applyQuestActionById(state, ALL, 'missing', 'start')).toBe(state);
  });

  it('starts a quest by id', () => {
    const state = applyQuestActionById(createDefaultCareerState(), ALL, 'simple', 'start');
    expect(getQuestStatus(state, SIMPLE_QUEST, ALL)).toBe('active');
  });

  it('completes a stage by id', () => {
    let state = applyQuestActionById(createDefaultCareerState(), ALL, 'simple', 'start');
    state = applyQuestActionById(state, ALL, 'simple', 'complete-stage', 'first');
    expect(getActiveStage(state, SIMPLE_QUEST)?.id).toBe('second');
  });

  it('no-ops a complete-stage action with no stageId', () => {
    const started = applyQuestActionById(createDefaultCareerState(), ALL, 'simple', 'start');
    expect(applyQuestActionById(started, ALL, 'simple', 'complete-stage')).toBe(started);
  });
});
