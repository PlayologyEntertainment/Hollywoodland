...
describe('settings', () => {
  ...
  it('clamps text scale and rejects invalid property types', () => {
    expect(normalizeSettings({ textScale: 9, highContrast: 'yes' })).toEqual({
      ...DEFAULT_SETTINGS,
      textScale: 1.5,
    });
  });
  ...
  it('round-trips normalized settings through storage', () => {
    ...
    repository.save({ ...DEFAULT_SETTINGS, textScale: 1.3, reducedMotion: true });
    expect(repository.load()).toEqual({ ...DEFAULT_SETTINGS, textScale: 1.3, reducedMotion: true });
  });
});
