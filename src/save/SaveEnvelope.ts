...
export function validateSaveEnvelope(value: unknown): asserts value is SaveEnvelope {
  ...
  if (typeof value.contentVersion !== 'string' || value.contentVersion.length === 0) {
    throw new Error('Save content version is missing.');
  }
  if (typeof value.saveId !== 'string' || value.saveId.length === 0) {
    throw new Error('Save ID is missing.');
  }
  ...
  if (
    ...
    !Number.isFinite(value.playtimeSeconds) ||
    value.playtimeSeconds < 0
  ) {
    throw new Error('Save playtime is invalid.');
  }
