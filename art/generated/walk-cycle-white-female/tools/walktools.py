...
def cream_mask(sub):
    """Sky-blue blouse pixels (used as the torso reference; kept the name `cream_mask` so the rest of this
    ported pipeline, which calls it by that name throughout, needs no other changes). Measured on the
    accepted idle frame: blouse fill clusters at (170-180, 180-195, 210-220) -- b is the highest channel and
    the palette is bright, unlike the much darker navy trousers (about (40, 50, 70)) or the warm skin tone
    (b is the *lowest* channel there)."""
    c = sub.astype(int)
    r, g, b, a = c[..., 0], c[..., 1], c[..., 2], c[..., 3]
    return (a > 200) & (b > r + 15) & (b > 150) & (g < b) & (r > 80) & (r < 225)
