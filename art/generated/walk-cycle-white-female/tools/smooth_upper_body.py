...
Reads  white-female-walk-master-unsmoothed.png  (output of build_sheet.py)
Writes white-female-walk-master-smoothed.png  (rig_arms.py then writes white-female-walk-master.png)
...
LOOP = 16
BOB_HALF_SHEET_PX = 4.3     # half of the bob's peak-to-peak: 4.3 sheet px = about 2 display px each way (4 px peak to peak); reused
...
def measure(c):
    """Blouse line y, and the torso's median x just below it (same rule as walktools.cream_mask, kept as
    its own inline copy here just like the original file did)."""
    c = c.astype(int)
    al = c[..., 3] > 24
    rows = np.where(al.any(axis=1))[0]
    h = rows.max() - rows.min() + 1
    blouse = (c[..., 3] > 200) & (c[..., 2] > c[..., 0] + 15) & (c[..., 2] > 150) & (c[..., 1] < c[..., 2]) & (c[..., 0] > 80) & (c[..., 0] < 225)
    ys = int(np.where(blouse.sum(axis=1) > 0.057 * h)[0].min())
    xs = np.where(blouse[ys:ys + 80])[1]
...
def main():
    sheet = np.array(Image.open(D + 'white-female-walk-master-unsmoothed.png').convert('RGBA'))
    ...
    Image.fromarray(out, 'RGBA').save(D + 'white-female-walk-master-smoothed.png')
