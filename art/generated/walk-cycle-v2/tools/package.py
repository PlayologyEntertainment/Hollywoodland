...
scale = round(202.4 / (sum(heights) / len(heights)), 3)     # keeps the on-screen figure about as tall as the old one (482 px x 0.42)
cfg = {'sheet': 'assets/characters/aspiring-actor-walk.webp', 'columns': 4, 'frameWidth': meta['cellW'], 'frameHeight': meta['cellH'],
...
json.dump(cfg, open(D + 'walk-cycle.json', 'w'), indent=2)
json.dump(meta, open(D + 'walk-cycle-build-record.json', 'w'), indent=1)
...
print('webp %d bytes; scale %s; strideWorld %s; %.2f cycles/s and %.1f frame changes/s at 390 px/s' % (
    os.path.getsize(D + 'aspiring-actor-walk-v2.webp'), scale, cfg['strideWorld'], 390 / cfg['strideWorld'], 16 * 390 / cfg['strideWorld']))
