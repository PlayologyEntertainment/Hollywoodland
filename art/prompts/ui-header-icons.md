...
- **Markup** (`index.html`): `#film-mode` and `#fullscreen` are `class="text-button icon-button"`, each holding one `<img alt="">` (decorative; the accessible name is the button's own `aria-label`). No `title` attribute: an earlier pass added one alongside `aria-label` as a fallback, but that draws the browser's own plain tooltip on top of the on-theme CSS one below, so it was removed. `#status-button` (Career), and the footer's `#advance-time`/`#return-menu`, are now `class="chrome-button deco-label"` instead of the header's old `.text-button`/plain `.deco-label` — one shared class for "the small rounded-rectangle button," so Career visually matches Wait/Menu exactly.
