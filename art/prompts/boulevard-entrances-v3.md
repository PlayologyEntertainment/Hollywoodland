...
**Other notes:** the Monarch gate's iron bars were transparent, so Plane 3 showed through; the gate was reworked on 2026-09-19 with the lot painted in (section 15). The Klieg Light's spotlight blade sign overhangs the module's right edge by design. The depot and alley have transparent gaps by design (open bays, alley sky).
...
**Behavior:** the scene draws each plane at its manifest scale (no stretch), places the buildings, and swaps four buildings to their active texture from the career state (`activeWhen`: The Gilded Spoon morning or evening; The Celestial Palace evening; Sunset Casting Exchange morning or afternoon; the Monarch gate once `discoveredCastingOffice` is set). The four entrances with no scene yet (The Silver Thimble, alley, The Celestial Palace, The Klieg Light) showed their sign but had no prompt; on 2026-09-19 the Thimble, the Palace and the Klieg Light gained interiors and became enterable (`location-interiors-v1.md`), so only the alley has none. Where trigger zones overlap the nearer door wins. The extras corral and soundstage stay playable as two trigger zones inside the Monarch module until the studio-lot map exists.
...
## 15. Monarch gate rework: the studio lot and the sound stage (2026-09-19)

Owner tweak to the approved Phase 1 spike: the Monarch Pictures gate did not read as a studio entrance because nothing was visible beyond it. It needed a studio lot behind the gate and along its sides, and a sound stage building to the right of it.

**Decisions (owner, 2026-09-19).** Two takes were staged first (`monarch-gate-v2-lot-a-master.png`, `-b-master.png`, with `-preview-over-sky.png` composites), both keeping the module's footprint and painting the lot behind the walls. The owner chose **take b** and asked for a **wider module so the sound stage stands on the street**. Take b is therefore the design reference of the wide render; the two narrow takes are kept as the record.

**Wide render.** `gpt-image-2` via gg-image, `--background transparent --quality high --size 2048x1024`; the backend returned RGBA 1774x887. One take was generated (the first attempt failed with a network error and was retried once). References: 1 take b (gate design and lot to keep on the left), 2 `art/Hollywoodland_Location_Backlot.png`, 3 `art/Hollywoodland_Concept_Studio.png`, 4 the approved Sunset Casting Exchange module. The prompt also removes take b's painted sky patch inside the arch, so the sky plane shows behind the whole module.

```text
Use case: stylized-concept
Asset type: one WIDE building module of the main street wall (Plane 4) of a registered parallax environment for a 1935 Hollywood side-scrolling game: the Monarch Pictures studio entrance with a sound stage building beside it. It will be placed at the right-hand end of a street, beside other separately made modules. This is a rework of an approved module.
Input image roles: Image 1 is the APPROVED design of the gate portion: keep the gate exactly as it is there, including the cream stucco arched gateway with stepped pylons and gold finials, the blank sign panel, wall lanterns, the wrought-iron double gate with gold-tipped bars, the green-roofed guard booth with a lit window, the red-and-white barrier arm, the low flanking stucco walls with red terracotta tile caps and teal tile bases and blank wall panels, AND the studio lot visible behind it: the painted studio street with a western-town facade seen through the arch, the water tower, film light towers, bungalow roofs and palms above the left wall. Keep all of that on the LEFT part of the new module at the same proportions. What changes: the large sound stage that appeared behind the right wall in Image 1 now becomes a full-height building that STANDS ON THE STREET beside the gate, with its front facade at street level. Image 2 is a scene from inside the studio lot: use it for the look of the sound stage (a barrel-vaulted hangar, a huge stage door, wooden western street, film lights). Image 3 is a bird's-eye concept of the whole studio: use it for the shape and materials of the sound stages (cream plaster walls, curved gray-blue corrugated roofs, clerestory windows). Image 4 is an already-approved building from the same street: match its exact painting style, materials, warm sunlight from the upper right and relative scale, but do NOT copy the building.
Primary request: Render exactly one wide module, about twice as wide as it is tall, arranged left to right: (1) the studio gate portion from Image 1 (guard booth on the left, the arched gateway with its iron gate in the centre-left, a short stretch of low flanking wall to the right of the gate); (2) directly to the right of it, a monumental SOUND STAGE building standing on the street line, side by side with the gate, with its front facade at street level: a huge cream-plaster barrel-vaulted hangar facade with a curved gray-blue corrugated roof, a row of clerestory windows under the roofline, and at street level a very tall closed sliding stage door with a visible track and heavy iron hardware in the centre of the facade, plus a separate small human-scale entrance door beside the big door (the same size as the guard-booth door, with a small unlit lamp above it and a blank plate), blank rectangular number plates and a large blank sign panel above the big door, teal tile base matching the gate walls, wall lanterns matching the gate lanterns, and a red terracotta-tiled cornice or parapet tying it visually to the gate walls. The sound stage is clearly taller than the gate and reads as the biggest building in the module. Behind and above the roofline of the sound stage, a hint of the lot: a tall palm or two and the top of a film light tower. Keep the same vivid, romanticized, hand-painted Golden Age Hollywood storybook look: cream stucco, terracotta, jewel-toned trim, restrained red and gold accents, crisp ornamental shapes, painterly texture.
Composition/framing: strict flat frontal elevation as seen by a side-scrolling camera, with only subtle depth cues. Every part of the module stands on the same street line, with the base of the foundation flush with the bottom edge of the image and cut flat there. The left and right edges of the module are flat vertical cuts, like party walls, so the module can be set directly against neighbouring modules; the sound stage may be cut flat at the right edge. Draw no sidewalk, curb, road, ground plane or cast shadow outside the gate. The tallest element stays fully inside the top edge with a small margin. Scale: the guard-booth door and the sound stage's small entrance door are the human-scale reference and are the same size; the gate keeps the same size relative to its booth as in Image 1.
Constraints: fully transparent background with true alpha outside the painted module: nothing behind the outermost roofs, palms and towers, no sky, no sky gradient, no clouds and no hills. The view through the arch shows the lot only, with no separate painted sky patch: keep the top of the arch opening filled by the lot or by hazy warm architecture, not by blue sky. Every sign panel, marquee, plaque, number plate and painted sign is completely blank with no letters, numerals, symbols, logos or legible marks. Do not render people, silhouettes of people, crowds, characters, animals, vehicles, streetcars, bicycles, freestanding signs, banners, flags, benches, tables, chairs, loose props, litter, watermarks, borders, captions, or text. No glow, halo or aura around the module. Attached architectural details such as lanterns, gate hardware, track and hinges are allowed.
```

**What the module now shows.** Left to right: a palm and a light tower above the left wall, a water tower and bungalow roofs, the green-roofed guard booth, the arched gateway with the studio street, a western-town facade, palms and a light tower visible through the bars, then a full-height sound stage on the street line: a barrel-vaulted cream-plaster hangar with a gray-blue roof, a clerestory window band, a blank sign panel, a very tall closed sliding stage door on iron track, and a human-scale side door with a lamp. A palm and a light tower peek over its right end.

**Open-gate variant.** An edit of the wide master (`--edit-target`, size returned unchanged at 1774x887) with the prompt below: the gate leaves fold back against the pylons, the barrier arm rises to near-vertical, and the studio street shows through the open arch. Only a feathered rectangle around the entrance (master px 420,340 to 940,870, 24 px feather) is taken from the edit; everything outside it is identical to the base (verified: no changed pixels above the rectangle). **Superseded by section 17:** that rectangle faded the raised barrier arm, so the composite is now built from the per-pixel difference.

```text
Edit this building image. Keep the building, its framing, its position, its scale, its colors, and every architectural detail outside the entrance area exactly identical, pixel for pixel. Change only the entrance area to its active state: the two wrought-iron gate leaves in the arch are swung fully open and folded back against the pylons on either side of the arch opening, so the arch is open and the studio lot beyond it (the paved studio street, western-town facade, palms and light tower) is clearly visible through it, and the red-and-white striped barrier arm is raised to a near-vertical position beside the gate. The guard booth, the sound stage and everything else stay exactly as they are. Keep the transparent background transparent. Do not add text, letters, numerals, people, or objects. Every sign panel and marquee face stays completely blank.
```

**Calibration.** `art/generated/boulevard-v3/calibrate_module.py` (rebuilt; the original was never committed) and `compose_active.py`. The rebuilt script was validated by reproducing the previous gate's geometry exactly (1513x988 runtime, ground row 928, overlap 60, 1504 extruded columns). Reference door: the guard-booth door leaf, 626-845 master px (219) = 190 display px, scale 0.8676. Runtime 2,271 x 1,131 (1,514 x 714 display, plus the hidden 40 px overlap). The variant uses the same trim box (15,27,1760,850), so the two register exactly. Numbers are in `runtime/calibration.json` and `runtime/states.json`.

```text
asset_id: boulevard_v3_monarch_gate_lot_takes_a_b, boulevard_v3_monarch_gate_wide, boulevard_v3_monarch_gate_wide_active
asset_type: replacement Plane 4 building module (Monarch Pictures gate, studio lot, sound stage) and its open-gate state variant
prompt_or_brief: this section
reference_asset_ids: boulevard_v3_plane4_modules (monarch-gate, sunset-casting-exchange), Hollywoodland_Location_Backlot, Hollywoodland_Concept_Studio, monarch-gate-v2-lot-b-master
generation_tool_and_version: gpt-image-2 via gg-image (Codex ChatGPT backend), --background transparent, --quality high; wide render --size 2048x1024, edit at the master size
generation_date: 2026-09-19
raw_source_location: art/generated/boulevard-v3/modules/monarch-gate-v2-lot-a-master.png, monarch-gate-v2-lot-b-master.png (narrow takes), modules/monarch-gate-v3-wide-master.png (the module in use); overlays/monarch-gate-v3-wide-active-edit-master.png (raw edit) and overlays/monarch-gate-v3-wide-active-master.png (composite). The previous master, modules/monarch-gate-master.png, and its overlays are unchanged.
human_edits: deterministic processing only (trim, bottom extrusion, door-scale premultiplied resize, 40 display px overlap strip, alpha normalization, feathered entrance composite for the variant); no repainting
review_status: pending review of the wide render (design and layout chosen by the owner 2026-09-19); in the game for review
rights_or_license_notes: project-owned development generation; human rights/provenance review required
runtime_files: public/assets/environments/boulevard-v3/buildings/monarch-gate.webp and monarch-gate-active.webp (2271x1131 RGBA, lossy quality 90, alpha_quality 100); staging PNGs art/generated/boulevard-v3/runtime/monarch-gate.png and monarch-gate-active.png
```

**Layout changes this forced** (manifest JSON and the built-in default, kept identical):

| Item | Before | After |
|---|---|---|
| Module width (display) | 1,009 | 1,514 |
| World width | 7,453 | 7,958 |
| `backlot-gate` trigger x / arch sign | 6,827 / (6,826, 548), board 308x85 | 6,860 / (6,863, 551), board 326x89 |
| `extras-corral` trigger x (booth door) | 6,611 | 6,625 |
| `soundstage` trigger x | 7,117 (right wall) | 7,711 (the side door) |
| `soundstage` sign | none | "SOUND STAGE" at (7,458, 505), board 325x78 |
| `palm-right` x | 7,273 | 7,828 (clear of the side door) |
| Hills / distant-buildings `scrollFactor` | 0.18 / 0.42 | 0.16 / 0.38 |

The parallax factors were lowered so the hills and distant-buildings planes still cover the camera at the end of the longer street (the coverage test in `tests/boulevard-manifest.test.ts` requires it). Nothing about their artwork or positions at the start of the street changed.

**Checked.** 323 tests pass; `tsc` and the production build are clean. The game was driven in headless Chrome: a new career, walked to the gate and the sound stage; the gate, both signs, the side door and the door triggers line up, and the lot reads through the arch.

**Flags for the reviewer:**

- The sound stage's side door is about 84% of the height of the guard-booth door (183 vs 219 master px), so it is a little smaller than the other doors on the street.
...
## 17. Monarch gate: see-through bottom edge and faded barrier arm (2026-09-19)

Owner report: the Monarch Pictures gate and the sound stage both had a 10-15 px gap at the bottom that showed the parallax background, and the gate up and gate down states looked odd, as if fading out in the scene. Both turned out to be problems in my processing of the new master, not placement.

**1. See-through bottom edge (both states).** The generated master's bottom edge is not a flat cut: from about 14 master px above its lowest content the walls fade out raggedly (master rows 836-849, only 7% of columns still fully opaque at row 844; the door thresholds reach furthest). `calibrate_module.py` used the lowest visible pixel as the ground row and copied semi-transparent pixels downward, so about **57% of the columns were see-through over the bottom ~8 display px above the ground line**, plus the hidden strip beneath it. The Klieg Light, by comparison, is 88.5% opaque (the rest are its intended open gaps) right down to its ground row. The gate and the sound stage share one module, which is why both showed it.

Fix (no manifest `y` change, so every building still stands on the same ground line and the layout test still holds):

- The ragged rows are cropped off: the trim box now ends at master row 836 (`15,27,1760,836`), so the module stands **12 display px lower** than before, which is the "move down" the owner asked for. It closes the gap by hiding the ragged band, and the runtime image is now 2,271 x 1,113.
- `calibrate_module.py` now extrudes each column from its last **fully opaque** pixel (alpha >= 250) and forces the extruded alpha to 255, so a soft master edge can never leave transparency above the ground line again.
- Result: the bottom rows of the runtime image are about 99.5% opaque (the remaining 0.3% are the module's own transparent edge columns).
- The module's two signs move down the same 12 px with it: the arch sign (6,755, 551 to 563) and the SOUND STAGE sign (7,350, 505 to 517). Trigger x positions are unchanged.

**2. Faded raised barrier arm (open gate).** `compose_active.py` kept only a feathered rectangle (master px 420,340 to 940,870, 24 px feather) around the entrance. The raised arm rises through that rectangle's top and right edge, so it dissolved into the wall toward its top, with washed-out stripes; the rectangle also swapped re-rendered stone texture across the whole arch. The script now builds its mask from the **per-pixel difference** between the base and the edit (threshold 56, closed and grown 8 px, 3 px feather, limited to master px 410,300 to 960,870), so the gate leaves, the arm and the opening behind them come across whole and nothing else is touched. A higher threshold (72, 96) left holes in the arm's white stripes, which blended cream wall back in and faded it again, so 56 was chosen. The composite was rebuilt from the untouched raw edit (`overlays/monarch-gate-v3-wide-active-edit-master.png`).

**Checked** in headless Chrome, before and after, with the casting-office flag set so the gate opens: the closed state meets the sidewalk with no hazy band, and in the open state the arm is crisp and complete from the pylon base to its tip. The open and closed textures are still the same size, and 330 tests pass.

**Not changed:** the closed-gate art itself, and the gate-swap behavior (an instant texture swap, not a fade).
