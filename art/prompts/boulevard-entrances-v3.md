...
## 15. Monarch gate rework: the studio lot behind the gate (2026-09-19)

Owner tweak to the approved Phase 1 spike: the Monarch Pictures gate does not read as a studio entrance because nothing is visible beyond it. It needs a studio lot behind the gate and along its sides, and a sound stage building to the right of it.

**Design decision (no layout change).** The module keeps its footprint (about 1,009 x 619 display px), the door scale, the guard booth and the arch. The lot is painted **into the module** behind and above the walls, so the world width, the trigger positions and the Art Director layout are untouched. Through the arch, between the gate bars: a paved studio street, a western-town facade, a water tower and a second sound stage. Above the left wall: bungalow roofs, palms, a water tower and a light tower with a crane. To the right of the gate, above the right wall: a large barrel-vaulted sound stage (cream plaster, gray-blue arched roof, clerestory windows, closed sliding stage door, blank sign panel and number plates). The right wing is already the sound stage trigger (`soundstage` at master px 1200), so the building now sits where the interaction is. If the owner would rather have the sound stage as a full-height facade on the street, the alternative is a wider module (about 350-600 px more), which moves the world width, the right prop and the sound stage trigger.

**References (compressed JPG copies):** 1 the current module (`monarch-gate-master.png`, gate design to keep), 2 `art/Hollywoodland_Location_Backlot.png` (the lot beyond the gate), 3 `art/Hollywoodland_Concept_Studio.png` (sound stage shapes, water tower), 4 the approved Sunset Casting Exchange module (style and scale).

**Generation:** `gpt-image-2` via gg-image, `--background transparent --quality high --size 1536x1024`, two takes from the same prompt. Both returned RGBA 1536x1024.

```text
Use case: stylized-concept
Asset type: one building module of the main street wall (Plane 4) of a registered parallax environment for a 1935 Hollywood side-scrolling game. It will be placed on a street beside other separately made modules. This is a rework of an existing module.
Input image roles: Image 1 is the CURRENT Monarch Pictures gate module: keep its gate design exactly (cream stucco arched gateway with stepped pylons and gold finials, blank sign panel, wall lanterns, wrought-iron double gate with gold-tipped bars, green-roofed guard booth with a lit window, red-and-white barrier arm, low flanking stucco walls with red terracotta tile caps and teal tile bases, blank wall panels). Its weakness is that the arch and walls show nothing behind them. Image 2 is a scene from inside the studio lot: use it for what lies beyond the gate (a barrel-vaulted sound stage hangar, a wooden western street, a black water tower, film light towers, a sunlit paved studio street). Image 3 is a bird's-eye concept of the whole studio: use it for the shape and materials of the sound stages (cream plaster, arched gray-blue roofs, clerestory windows) and the water tower. Image 4 is an already-approved building from the same street: match its exact painting style, materials, warm sunlight from the upper right and relative scale, but do NOT copy the building.
Primary request: Render exactly one module: the Monarch Pictures studio gate WITH THE STUDIO LOT VISIBLE BEHIND IT. Keep the gate, pylons, arch, guard booth, barrier arm and flanking walls from Image 1 at the same proportions and positions. Now paint the studio behind them. (a) Through the arch, between the iron bars, show a sunlit paved studio street receding into the lot, lined with a wooden western-town facade on one side, a tall black wooden water tower with an unmarked round tank, film light towers with lamps, and the front of a second sound stage in the distance, all warm and slightly hazy so the gate reads in front. (b) Above and behind the LEFT flanking wall, show the tops of the lot: red terracotta-roofed office bungalows, a row of tall palms, a corner of the water tower, and a film light tower with a boom crane arm. (c) To the RIGHT of the gate, above and behind the right flanking wall, show a large SOUND STAGE building: a huge cream-plaster barrel-vaulted hangar with a curved gray-blue corrugated roof, a row of clerestory windows under the roof edge, a massive closed sliding stage door, a blank rectangular sign panel on its facade, and blank number plates beside the door. The sound stage is taller than the gate's flanking wall and rises clearly above it; the wall stays in front of its lower part. (d) The roofs, palms, tower and sound stage are part of the module, painted opaque, so the wall wings no longer read as bare screens. Keep the same vivid, romanticized, hand-painted Golden Age Hollywood storybook look: cream stucco, terracotta, jewel-toned trim, restrained red and gold accents, crisp ornamental shapes, painterly texture.
Composition/framing: strict flat frontal elevation as seen by a side-scrolling camera, with only subtle depth cues. The gate, walls and booth are the foreground layer and the lot is a lighter, slightly hazier layer behind them. The left and right edges of the module are flat vertical cuts, like party walls, so the module can be set directly against neighbouring modules; the sound stage may be cut flat at the right edge. The base of the foundation sits flush with the bottom edge and is cut flat there. Draw no sidewalk, curb, road, ground plane or cast shadow outside the gate. The tallest element stays fully inside the top edge with a small margin. Scale: the guard-booth door is the human-scale reference and stays the same size relative to the gate as in Image 1.
Constraints: fully transparent background with true alpha outside the painted module: nothing behind the outermost roofs, palms and sound stage, no sky, no sky gradient, no clouds and no hills. Every sign panel, marquee, plaque, number plate and painted sign is completely blank with no letters, numerals, symbols, logos or legible marks. Do not render people, silhouettes of people, crowds, characters, animals, vehicles, streetcars, bicycles, freestanding signs, banners, flags, benches, tables, chairs, loose props, litter, watermarks, borders, captions, or text. No glow, halo or aura around the module. Attached architectural details such as lanterns, gate hardware and window frames are allowed.
```

```text
asset_id: boulevard_v3_monarch_gate_lot_a, boulevard_v3_monarch_gate_lot_b
asset_type: replacement building module for Plane 4 (Monarch Pictures gate with the studio lot behind it), transparent-background source art
prompt_or_brief: this section
reference_asset_ids: boulevard_v3_plane4_modules (monarch-gate), Hollywoodland_Location_Backlot, Hollywoodland_Concept_Studio, boulevard_v3_plane4_modules (sunset-casting-exchange)
generation_tool_and_version: gpt-image-2 via gg-image (Codex ChatGPT backend), --background transparent, --quality high, --size 1536x1024
generation_date: 2026-09-19
raw_source_location: art/generated/boulevard-v3/modules/monarch-gate-v2-lot-a-master.png and monarch-gate-v2-lot-b-master.png (the previous master, monarch-gate-master.png, is unchanged); previews composited over the game sky: monarch-gate-v2-lot-a-preview-over-sky.png and -b-
human_edits: none (the previews are composites for review only)
review_status: pending review (take a or b not yet chosen)
rights_or_license_notes: project-owned development generation; human rights/provenance review required
runtime_files: not yet promoted; the running game still uses the previous gate
```

**Verified from the files:** both are RGBA 1536x1024 with alpha 0 at all four corners. The dark glow some viewers show around them is leftover color stored under fully transparent pixels, not visible in the alpha channel or over a real background (see the previews). No baked text, numerals or people in either.

**Take a:** the sound stage is the larger of the two, with its rail-hung sliding door, a window strip and a blank panel plainly visible. The lot through the arch is pale and hazy, and the game sky shows through the top of the arch, so it merges with the sky plane. The left side has the water tower, a light tower with a crane, bungalow roofs and palms.

**Take b:** the view through the arch is the more vivid and legible (a western-town facade, palms, a light tower, a distant domed stage), but it contains its own painted patch of blue sky that will sit against the sky plane. The sound stage is slightly smaller and cut by the right edge, with a palm beside it. The left side has a water tower, a light tower, bungalow roofs and a large palm.

**Not done yet (waiting on the owner's pick):** calibration to door scale (the guard-booth door is the reference, 190 display px), the open-gate variant (`monarch-gate-active`, made from the chosen master), WebP conversion, and the manifest. A finished module also has to keep the gate at the same x so the trigger and sign positions stay valid.
