...
**Open-gate variant.** An edit of the wide master (`--edit-target`, size returned unchanged at 1774x887) with the prompt below: the gate leaves fold back against the pylons, the barrier arm rises to near-vertical, and the studio street shows through the open arch. Only a feathered rectangle around the entrance (master px 420,340 to 940,870, 24 px feather) is taken from the edit; everything outside it is identical to the base (verified: no changed pixels above the rectangle).
