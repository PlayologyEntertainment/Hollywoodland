...
Arm rig (added after owner feedback that the arms were jittery): the generator drew the same scissor in every frame (one hand back at about -16 to -33 degrees, the other forward at about +38 to +52, never crossing, not even in the passing frames), with the hands wandering up to about 50 sheet px between adjacent frames. `tools/rig_arms.py` (with `tools/arm_parts.py`) rebuilds each loop frame as three layers: the far arm behind the body, the torso layer with both drawn arms removed and the shirt back repainted, and the near arm in front. Both arms are cut from the standing pose, where they hang straight, in two rigid pieces (sleeve with cuff, forearm with fist), given a clean smooth outline, and swung on a pendulum: the near arm goes back while the far arm goes forward, they pass beside the body at the passing frames (4-6 and 12-14), then swap; each arm makes one full swing per loop, opposite to the leg on its side. Parameters at the top of the tool: mean 0 degrees, swing 30 degrees each way (about +30 forward, -30 back; the first version was +33 / -23 and the owner found it too far forward), elbow bend 12 degrees at rest plus 0.55 degrees per degree of forward swing (about 28 degrees when forward), far arm drawn 10% darker. The largest change between adjacent frames is about 11 degrees of arm swing, at the moment the arms pass each other. Torso layer (revised after review found the belt area disappearing and the shoulder ragged): erasing the old arms also removed the belt, shirt hem, chest front and trouser corners drawn under or against them (the merged version kept as little as 35-50% of the belt and hip band in some frames). The torso layer now (1) keeps the drawn shoulder cap of the near sleeve, a disc of 40 px around the shoulder pivot, with its own outline and the suspender strap over it, and fades the rigged upper arm in from under it so the arm grows out of the shoulder; (2) restores belt and trouser pixels caught only in the 3 px erase margin outside the arm's own outline ring; (3) repaints only the area the erase cleared, meaning the exposed shirt back, the chest front and the belt ends, with silhouette edges anchored to the visible belt ends and drawn as anti-aliased lines, shirt tone and grain matched to the frame's own shirt and blended from the neighbouring drawn pixels; (4) patches small enclosed pinholes. Measured on the torso layer alone (no arms drawn), the belt and hip band now keeps 100-108% of the pre-rig dark pixels in every frame; where the composite shows less, that is the new arm legitimately crossing in front of the hip at the passing frames.

Legs, head and idle frame are unchanged (verified: 0 changed pixels in the head rows and below the hips). The pre-rig sheet is kept as `aspiring-actor-walk-v2-master-smoothed.png`.
...
- **The arms are a rig, not drawings from the generator.** Both are the same arm cut from the standing pose, so they are identical in shape and only the far one is darker, the sleeve and forearm do not squash or twist, and the shirt back behind the near arm is repainted (flat tone, drawn edge). It is clean at game size; the kept shoulder cap and the repainted shirt back are the places to look if the sheet is ever shown larger, and the arms pass in front of the hips at the passing frames, so the belt is partly covered there by design.
